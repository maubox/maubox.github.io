...
onkeydown = onkeyup = function (e) {
  ...
  if (asking) {
    ...
    if (map[90]) {
      ...
      check(12);
      console.log('alt+v')
    } else if (map[65] || map[83] || map[68] || map[70] || map[81] || map[87] || map[69] || map[82]) {
      // all misclicks
      check(1000);
      console.log('hiey')
    }
...
const check = x => {
  ...
  for (var i = 0; i < entry.length; i++) {
    ...
    list += '<li><img width="50" height="50" src="' + entry[i].painting + '"/>: <img width="50" height="50" src="' + rice + '"/>' + entry[i].time + '</li>';
  }
  ...
  if (entry.length < questions) {
    ...
    document.getElementById('time').innerHTML = "correct: " + totalCorrect + "/" + entry.length
